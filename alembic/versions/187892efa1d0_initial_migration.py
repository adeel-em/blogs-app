...
def upgrade():
    ...
    op.create_table(
        ...
        sa.Column("tags", sa.String(), nullable=False, default=""),
        sa.Column("created_at", sa.DateTime(), nullable=False, default=sa.func.now()),
        sa.Column(
            "updated_at",
            sa.DateTime(),
...
def downgrade():
    ...
    result = connection.execute(
        text("SELECT 1 FROM pg_indexes WHERE indexname = 'ix_blogs_title'")
    )
    if result.scalar():
        op.drop_index("ix_blogs_title", table_name="blogs")

    result = connection.execute(
        text("SELECT 1 FROM pg_indexes WHERE indexname = 'ix_comments_title'")
    )
    if result.scalar():
        op.drop_index("ix_comments_title", table_name="comments")

    result = connection.execute(
        text("SELECT 1 FROM pg_indexes WHERE indexname = 'ix_users_title'")
    )
    if result.scalar():
